...
#define MAXRETRIES              50           // number of receive attempts before giving up
void I2C_Init(void){
...
uint8_t I2C_Recv(int8_t slave){
  ...
  uint8_t toReturn = I2C3_MDR_R&0xFF;          // usually returns 0xFF on error
	return toReturn;
}
  ...
  do{
    ...
    I2C3_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_STOP    // no stop
                         ...
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    Buff[0] = (I2C3_MDR_R&0xFF);       // MSB data sent first
    I2C3_MCS_R = (0
                         | I2C_MCS_ACK      // positive data ack
                       //  & ~I2C_MCS_STOP    // no stop
                         | I2C_MCS_START    // generate start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    Buff[1] = (I2C3_MDR_R&0xFF);       // LSB data sent last
    I2C3_MCS_R = (0
                         | I2C_MCS_ACK      // positive data ack
                       //  & ~I2C_MCS_STOP    // no stop
                         | I2C_MCS_START    // generate start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    Buff[2] = (I2C3_MDR_R&0xFF);       // MSB data sent first
    I2C3_MCS_R = (0
                         | I2C_MCS_ACK      // positive data ack
                       //  & ~I2C_MCS_STOP    // no stop
                         | I2C_MCS_START    // generate start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    Buff[3] = (I2C3_MDR_R&0xFF);       // MSB data sent first
    I2C3_MCS_R = (0
                         | I2C_MCS_ACK      // positive data ack
                       //  & ~I2C_MCS_STOP    // no stop
                         | I2C_MCS_START    // generate start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    Buff[4] = (I2C3_MDR_R&0xFF);       // MSB data sent first
    I2C3_MCS_R = (0
                       //  & ~I2C_MCS_ACK     // negative data ack (last byte)
                         | I2C_MCS_STOP     // generate stop
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    Buff[5] = (I2C3_MDR_R&0xFF);       // MSB data sent first
    retryCounter = retryCounter + 1;        // increment retry counter
  }                                         // repeat if error
  while(((I2C3_MCS_R&(I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0) && (retryCounter <= MAXRETRIES));
}
...
uint32_t I2C_Send6(int8_t slave, unsigned char *Buff){
  ...
  I2C3_MDR_R = Buff[1]&0xFF;         // prepare second byte
  I2C3_MCS_R = (0
                     //  & ~I2C_MCS_ACK     // no data ack (no data on send)
                     //  & ~I2C_MCS_STOP    // no stop
                     //  & ~I2C_MCS_START   // no start/restart
  ...
  I2C3_MDR_R = Buff[2]&0xFF;         // prepare third byte
  I2C3_MCS_R = (0
                     //  & ~I2C_MCS_ACK     // no data ack (no data on send)
                     //  & ~I2C_MCS_STOP    // no stop
                     //  & ~I2C_MCS_START   // no start/restart
  ...
  I2C3_MDR_R = Buff[3]&0xFF;         // prepare fourth byte
  I2C3_MCS_R = (0
                     //  & ~I2C_MCS_ACK     // no data ack (no data on send)
                     //  & ~I2C_MCS_STOP    // no stop
                     //  & ~I2C_MCS_START   // no start/restart
  ...
  I2C3_MDR_R = Buff[4]&0xFF;         // prepare fifth byte
  I2C3_MCS_R = (0
                     //  & ~I2C_MCS_ACK     // no data ack (no data on send)
                     //  & ~I2C_MCS_STOP    // no stop
                     //  & ~I2C_MCS_START   // no start/restart
  ...
  I2C3_MDR_R = Buff[5]&0xFF;         // prepare sixth byte
  I2C3_MCS_R = (0
                    //   & ~I2C_MCS_ACK     // no data ack (no data on send)
                       | I2C_MCS_STOP     // generate stop
                    //   & ~I2C_MCS_START   // no start/restart
...
void I2C_SendList(unsigned char *List){
  unsigned char listcount = *List++;

  while (listcount) {
		uint8_t slave = *List;
		List++;
		uint8_t data1 = *List;
    I2C_Send1(slave, data1);
    List ++;
    listcount--;
  }
}
